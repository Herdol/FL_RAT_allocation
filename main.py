...
if __name__ == "__main__":
    ...
    for fl in range(config.FL_cycles):
        ...
        for widx in range(num_workers):
            ...
            policy_net.load_state_dict(checkpoint_dict['model'])
            optimizer.load_state_dict(checkpoint_dict['optimizer'])
            policy_net.eval()
            print('Globel model {} for worker {} is loaded'.format(checkpoint_dict['epoch'],widx))
            # Profiling training
            ...
            train(policy_net,widx,Fl_cycle=fl,algorithm=algorithm,target=target_net)
            # Upload model
    ...
    model= policy_net
    train(model,widx,Fl_cycle=fl,algorithm=algorithm,target=target_net)
